...
describe("agent capabilities", () => {
  ...
  it.effect("issues board-only credentials when agent browser access is off", () =>
    Effect.gen(function* () {
      const issued = yield* startSessionWith(false, asThreadId("thread-browser-off"));

      assert.deepEqual([...issued[0]!.capabilities!], ["board"]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  it.effect("adds preview capability when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual([...issued[0]!.capabilities!], ["board", "preview"]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
});
