  ...
  it.effect("keeps the projection thread schema aligned with repository writes", () =>
    Effect.gen(function* () {
      ...
      const migrated = yield* threads.getById({
        threadId: ThreadId.make("thread-before-039"),
      });
      ...
      assert.equal(migratedRow.titleRegenerationRequestId, "title-request-before-039");
      assert.equal(migratedRow.titleRegenerationStartedAt, "2026-08-04T00:01:00.000Z");

      yield* threads.upsert({
        threadId: ThreadId.make("thread-after-039"),
        ...
        snoozedAt: null,
        workflowLane: null,
        titleRegenerationRequestId: CommandId.make("title-request-after-039"),
