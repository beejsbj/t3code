import { LaneId, ProjectId, ThreadId, ProviderInstanceId } from "@t3tools/contracts";
  ...
  it.effect("round-trips non-null settlement values through the thread row", () =>
    Effect.gen(function* () {
      ...
      yield* threads.upsert({
        ...
        workflowLanePlacedAt: "2026-03-25T00:00:00.000Z",
        workflowLanePlacementReason: "Groomed while the agent was shaping it",
        latestUserMessageAt: null,
        pendingApprovalCount: 0,
        pendingUserInputCount: 0,
        hasActionableProposedPlan: 0,
      ...
      assert.strictEqual(row.workflowLane, "review");
      ...
      assert.strictEqual(updated?.workflowLane, null);
    }),
  );
});
